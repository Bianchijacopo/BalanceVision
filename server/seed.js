...
async function seed() {
  ...
  const existing = await get('SELECT id FROM users WHERE email = ?', ['admin@gmail.com']);
  if (existing) {
    console.log('Account admin gia esistente (id: %d, email: admin@gmail.com)', existing.id);
  } else {
    const hash = bcrypt.hashSync('admin', 10);
    const result = await run('INSERT INTO users (email, password_hash, name, created_at) VALUES (?, ?, ?, ?)', ['admin@gmail.com', hash, 'Admin', localNow()]);
